...
if command -v jq >/dev/null 2>&1 && [[ -f "$SERVICES_JSON" ]]; then
    ...
    # exclude: всё, что НЕ vps и НЕ auto (dpi/direct идут мимо туннеля).
    # include: vps (строгий пол, без DPI-групп) и auto (пол + микс —
    # семантика владельца 2026-09-18: auto = две подложки, 100% работоспособность).
    jq -r '.[] | select((.mode // "zapret")!="vps" and (.mode // "zapret")!="auto") | .domains[]?' "$SERVICES_JSON" 2>/dev/null > "$EXCLUDE_FILE" || true
    jq -r '.[] | select(.mode=="vps" or .mode=="auto") | .domains[]?' "$SERVICES_JSON" 2>/dev/null > "$INCLUDE_FILE" || true
