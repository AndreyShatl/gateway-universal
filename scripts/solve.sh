...
if [ "$TEST_MODE2" = 1 ]; then
  ...
  # T-zapret2-fallthrough: правило-1 (QNUM/QNUM_UDP, очередь zapret1) без
  # слушателя в этом режиме — --queue-bypass у NFQUEUE без слушателя это
  # терминирующий ACCEPT (см. правку выше по основному потоку), поэтому оно
  # молча ACCEPT'ит все пакеты домена, не доходя до правила-2 (наша Z2QNUM).
  # Без удаления этот тестовый режим гарантированно не увидит ни одного
  # пакета независимо от домена/стратегии — воспроизведено живым тестом
  # (iptables -v счётчики: правило-1 съедало весь трафик, правило-2 = 0).
  purge_test_mangle_rules $QNUM_TCP $QNUM_UDP
...
# T-zapret2-fallthrough (2026-08-12): зачистка правила-1 (QNUM/QNUM_UDP, очередь
# zapret1) ПОСЛЕ того, как zapret-тиры выше закончили пробовать — обнаружен
# живым тестом реальный баг: --queue-bypass у NFQUEUE без слушателя ведёт себя
# как терминирующий ACCEPT (проверено iptables -v счётчиками пакетов на самом
# правиле), а НЕ "пропусти дальше по цепочке", как предполагал комментарий тут
# раньше. try_tier выше убивает nfqws1 в конце каждой попытки — но само
# mangle-правило на позиции 1 остаётся висеть и с этого момента молча
# ACCEPT'ит КАЖДЫЙ пакет домена, даже не пытаясь дойти до правила-2 (очередь
# zapret2 ниже). Итог — zapret2-тир НИКОГДА не получал ни одного пакета в
# продовом переборе с момента добавления движка (T-zapret2, 2026-07-29): не
# баг стратегий, баг порядка mangle-правил. ciadpi ниже эту цепочку вообще не
# использует (свой TPROXY-механизм) — удаление здесь его не касается.
purge_test_mangle_rules $QNUM $QNUM_UDP
