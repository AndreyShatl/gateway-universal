...
**Синтетический тест наконец поймал живой сигнал (2026-09-07):** прямой curl со шлюза на заблокированные
`x.com`/`www.facebook.com` (6+3 пробы, все connection failed) — `x.com` пойман ОБОИМИ путями:
eBPF (боевой) в 14:58:37 классифицировал как кандидат, но НЕ добавил из-за ошибки prober
(`direct=dns: lookup x.com on 127.0.0.1:53: i/o timeout` — DNS-резолв пробера через локальный
резолвер упал в таймаут, кандидат тихо потерян); pcap-тень в 15:00:05 выдала
`no-response-after-clienthello → живой сигнал провала`. Facebook (6 проб) НЕ пойман ни одним
путём — открыто (возможные причины: блок по RST-на-SYN вне сигнатур, либо пороги/окно не
набраны при 8с-интервале проб). Т.е. сверка "секунда в секунду" снова НЕ сошлась по времени и
классификации — тот же открытый класс, что `93.158.177.1`.

**Баг, найденный тестом (исправлен 2026-09-07, задеплоен на 132):** теневой `watch` без
`--apply` РЕАЛЬНО мутировал прод — в ветке isBrainEntity/inAutoroute `addToSet` и
`brain-apply.sh vps` вызывались без проверки флага apply (тень залогировала "переведён на VPS"
и исполнила brain-apply; выручило только то, что x.com уже был в autoroute от brain — изменений
не произошло). Фикс: `maybeRetriggerBrainEntity(domain, source, apply)` + гейтинг addToSet;
в тени теперь только лог "🟡 тень: ... БЫ перевёл на VPS". Контрольный прогон: md5
autoroute.json до/после идентичен. Осталось открытым: (а) prober теряет кандидатов при DNS-
таймауте локального резолвера — нужен fallback-резолвер (1.1.1.1 напрямую); (б) Facebook-сигнатура
не ловится; (в) расхождение времени/классификации pcap↔eBPF на одном трафике.
...
### T-adguard · Блокировка рекламы на уровне DNS (AdGuard Home) · H · done
Владелец спросил, есть ли способ убрать рекламу на всех устройствах дома. Выбрано: AdGuard Home
на самом шлюзе (не браузерное расширение, не Pi-hole) — работает для ВСЕХ устройств сразу, без
установки чего-либо на телефоны/компьютеры, хорошо сочетается с уже настроенным dnscrypt-proxy.
**Архитектура:** клиент → AdGuard Home (`:53`, фильтрует рекламу/трекеры) → dnscrypt-proxy
(`127.0.0.1:5353`, шифрованный upstream, был сдвинут с `:53` через `dnscrypt-proxy.socket.d/
override.conf`) → интернет. Существующий редирект (`gateway-dns-redirect.sh`, весь DNS-трафик
LAN → порт 53 локально) НЕ менялся — AdGuard Home просто занял порт 53 вместо dnscrypt-proxy,
редирект прозрачно продолжил работать.
**Установка:** бинарь скачан через VPS-туннель (`static.adguard.com` заблокирован DPI напрямую —
TLS зависал без ответа, классический паттерн этого проекта). `AdGuardHome -s install` +
настройка через `/control/install/configure` API (без интерактивного мастера) — DNS `:53`, web-UI
`:3000`, upstream `127.0.0.1:5353`. Включены 2 стандартных фильтра (AdGuard DNS filter 160k+
правил, AdAway 6.5k) — оба подтверждённо блокируют живьём (`doubleclick.net`/
`googleadservices.com` → `0.0.0.0`).
**Веб-UI `:3000` заперт на LAN** — тот же паттерн, что у gateway-ui `:8088` (`AdGuardHome.service.d/
firewall.conf`, ACCEPT LAN + DROP всё остальное).
**Побочно найдено и закрыто:** ACCEPT-для-LAN-потом-DROP-всё-остальное паттерн (используется и у
gateway-ui:8088, и теперь у AdGuardHome:3000) НЕ пропускал loopback (127.0.0.1 не входит в
192.168.0.0/16) — сам шлюз не мог обратиться к своим же локальным API. Добавлено явное
`-A INPUT -i lo -j ACCEPT` (персистентно, `/etc/iptables/rules.v4`) — общий фикс, чинит доступ
к обоим сервисам с самого шлюза, не только к AdGuard.
**gateway-ui интеграция:** [gateway-ui/adguard.go](gateway-ui/adguard.go) — `/api/adguard`
(auto-login через `ADGUARD_PASSWORD` в config.env, сессия кэшируется в памяти, не логинимся на
каждый опрос — у AdGuard Home есть anti-bruteforce троттлинг повторных `/control/login`).
Раздел «Реклама (DNS)» в дашборде: сводка (запросов/заблокировано/% за 24ч) + топ заблокированных
доменов + кнопка на полную панель AdGuard Home (фильтры/клиенты/журнал запросов — не дублируем,
там уже полноценный интерфейс).
**Приёмка (де-факто, стенд, 2026-07-26):** `dig @192.168.1.132` — обычные домены резолвятся,
рекламные → `0.0.0.0`; `/api/adguard` отдаёт живую статистику (257 запросов, 28 заблокировано на
момент проверки, топ заблокированных — реальные fbcdn/doubleclick/appmetrica домены); все сервисы
(xray/zapret/gateway-ui/detector/brain-worker/dnscrypt-proxy/AdGuardHome) active, диск/память в норме.
**Откат:** `/etc/systemd/system/dnscrypt-proxy.socket.d/override.conf.bak-pre-adguard` (старый,
слушал :53 напрямую) — вернуть, остановить AdGuardHome, перезапустить dnscrypt-proxy.socket.

### T-no-vps · Режим "только zapret" — без VPS · H · done
Владелец отметил: у части будущих пользователей может не быть VPS вообще — раньше вся система
(discord-tproxy, статические сервисы mode=vps, авто-обход) молча предполагала его наличие.
Попросил: (1) вопрос при установке "есть ли VPS?", (2) переключатель в UI между "VPS+zapret" и
"только zapret" (для тех, у кого VPS есть, но хочется временно отключить).
**Без фолбэка (согласовано):** домены, которые zapret не пробивает, в режиме "только zapret"
остаются заблокированными как есть — честно, без притворства, что что-то работает.
**install.sh:** новый вопрос (`Есть ли у вас VPS...? [Y/n]`) перед VPS_ADDR-промптами — "нет"
выставляет `INSTALL_XRAY=no`, пропускает весь VPS-блок промптов/установки/discord-tproxy (уже
было условно на INSTALL_XRAY). Новое: без VPS все сервисы в `zapret-services.json` (обе точки
установки — основная и внутри секции "мозга") принудительно переводятся на `mode: "zapret"`
(`jq 'map(.mode = "zapret")'`) — без этого домены вроде Discord (mode=vps по умолчанию в шаблоне)
просто выпали бы из обхода целиком (не в zapret-хостлисте, нет xray для проксирования).
**Runtime-переключатель (для тех, у кого VPS ЕСТЬ, но хочется временно выключить):**
[iptables/vps-mode.sh](iptables/vps-mode.sh) — `{on|off}`, снимает/возвращает ДВА REDIRECT-правила
(LAN 80/443 → xray :12345, `gw_autoroute` → xray :12347); brain per-domain сущности (RETURN
carve-out, T44-46) это не затрагивает — они физически раньше в цепочке правил.
[systemd/vps-mode.service](systemd/vps-mode.service) — тот же паттерн, что game-mode.service,
переприменяет сохранённое состояние (`/etc/gateway/vps-mode.conf`) на каждой загрузке (CANON #19 —
live-команда не переживёт ребут сама по себе, обязателен systemd-юнит с восстановлением).
`scripts/brain-apply.sh`: `has_vps()` теперь проверяет ДВА условия — `VPS_ADDR` в config.env
задан И `vps-mode.conf` != "off". Команда `vps` (мозг предлагает VPS как fallback, когда zapret
не пробил) при `has_vps()`=false честно логирует "остаётся заблокирован" вместо создания
автообхода на несуществующий/выключенный xray-туннель (что раньше просто зависало бы).
`gateway-ui/vpsmode.go` — `/api/vps-mode` (GET/POST) + раздел «Режим работы» в дашборде
(сразу после «Подключение» — рядом с VPS-настройками, логично по месту).
**Найденный при тестировании баг (не концептуальный, опечатка):** `main_redirect -"$op"` с
`op="-D"` давало `--D` (двойной дефис) — iptables тихо не выполнял то, что просили. Второй баг:
`|| true` ВНУТРИ функции проверки (`-C`) скрывал её реальный результат — `on` никогда не доходил
до ветки `-I` (правило не вставлялось после `off`). Оба исправлены и проверены живьём: off снимает
оба правила, on восстанавливает, повторный on идемпотентен (не дублирует).
**Приёмка (де-факто, стенд, 2026-07-26):** `vps-mode.sh off/on` — правила снимаются/восстанавливаются
корректно; `brain-apply.sh vps <domain>` — при on уходит в автообход, при off явно логирует
"остаётся заблокирован"; `/api/vps-mode` GET/POST протестирован через реальный логин; xray/zapret/
gateway-ui/brain-worker остались active на всех этапах теста; финальное состояние возвращено в on
(боевой режим с VPS, как и должно быть на этом стенде).

## todo — хвосты / на будущее
- **install.sh не разворачивает AdGuard Home** (2026-07-26, T-adguard): установка была ручной
  (скачать бинарь через VPS, `-s install`, API-конфигурация, systemd drop-in для firewall) —
  для чистой установки на новую машину это тоже нужно будет автоматизировать отдельной задачей,
  как и с "мозгом" выше.
...
- ~~install.sh не разворачивает "мозг" вообще~~ — **закрыто 2026-08-16** (T-timer-chaining, см.
  Архитектура.md): install.sh теперь копирует ВСЕ `scripts/*.sh` (включая brain-воркеры и
  `zapret-auto-update.sh`) в `/opt/gateway-brain/` и ставит все brain-юниты/таймеры; чистая
  установка получает "мозг" целиком. Автопроверка полноты — `tests/`, ручная сверка при
  следующем деплое на чистую машину не помешает. (Историческая запись 2026-07-23: solve/
  brain-apply/brain-worker/brain-nightly жили только на стенде, не в `scripts/` репо.)
...
- ~~Instagram зависает~~ — **сделано 2026-07-23**: `instagram.mode` был `zapret` (кто-то переключил
  ...
  2026-07-23. **Развитие (зафиксировано задним числом 2026-09-07, см. DECISIONS 2026-09-07):**
  с ~2026-08-17 Instagram работает через per-domain brain-группу zapret (не статический
  mode=zapret!) после T-cdn-refresh — инвариант #18 для СТАТИЧЕСКИХ TCP-сервисов остаётся в силе.
  Открытый вопрос на будущее: сделать `mode: "zapret"` рабочим для TCP-каналов
