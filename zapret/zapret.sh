...
build_proto() {
    ...
    done < <(jq -r --arg p "$proto" '.[] | select((.mode // "zapret")=="zapret") | .id as $id | .channels[] | select(.proto==$p) | [$id, .ports, (.l7 // ""), .desync] | join("\u001f")' "$SERVICES")
...
start() {
    ...
    local sid
    jq -r '.[] | select((.mode // "zapret")=="zapret") | select((.domains|length)>0) | .id' "$SERVICES" | while read -r sid; do
