...
23. Паттерн "ACCEPT для LAN-подсети позицией 1, затем DROP всё остальное на этот же порт" (используется у `gateway-ui:8088`, теперь и у `AdGuardHome:3000` для запирания веб-панели на LAN) НЕ пропускает loopback (`127.0.0.1` не входит в `192.168.0.0/16`) — сам шлюз не может обратиться к своим же локальным API/портам. Нужен явный `-A INPUT -i lo -j ACCEPT` РАНЬШЕ этого паттерна (персистентно и в `/etc/iptables/rules.v4`, см. инвариант #19) — общее правило, а не per-порт костыль. Найдено 2026-07-26 при добавлении AdGuard Home (не мог залогиниться в свой же control API через 127.0.0.1).
