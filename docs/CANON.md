...
- **brain-apply.sh** ([../scripts/brain-apply.sh](../scripts/brain-apply.sh)) — применитель, proto-осведомлённый (T57). Модель **«очередь = сервис = сущность»**: каждый домен получает свой ipset `brain_<domain>` и свои iptables-правила (`svc_rules`, proto-ветка):
  ...
  2. `mangle POSTROUTING NFQUEUE` с `--queue-bypass` на своей очереди (`alloc_queue`, brain-apply.sh:24, аллоцирует от 210+) — сам десинхрон. **Пропускается для accept-only** (пустая стратегия, только UDP) — очередь/nfqws не нужны, только ACCEPT-правила.
  3. подрежимы `zapret`/`vps`/`remove` также ведут/чистят домен в VPS-автообходе (`ar_add`/`ar_del`) — mode `zapret` явно убирает домен из VPS.
  - `restore` пересоздаёт все сущности (включая accept-only) из `/etc/gateway/brain-services.json` после ребута (transient-правила/демоны не переживают reboot).
  - **Инвариант:** `do_remove` обязан звать `svc_rules -D` ВСЕГДА, а не только при наличии очереди — иначе accept-only сущности при удалении оставляют ACCEPT-правила в iptables навсегда (была утечка, закрыта в T57).
- **brain-worker.sh** ([../scripts/brain-worker.sh](../scripts/brain-worker.sh)) — потребитель очереди `/etc/gateway/brain-queue` (atomic pop через flock, строка `domain\tsource`), для каждого домена: whitelist-guard → `solve.sh domain source` → `brain-apply.sh zapret <domain> <proto> <args>` (ZAPRET → сущность+убрать из VPS; VPS → автообход; DIRECT → пропуск/GC).
- **brain-nightly.sh** ([../scripts/brain-nightly.sh](../scripts/brain-nightly.sh)) — ночной проход: складывает в очередь воркера ВСЕ управляемые домены (сущности из `brain-services.json` + VPS-автообход домены, не покрытые featured-сервисом) на переоценку, source=`reeval` (полный перебор, без классификационного пропуска). **Важно:** это НЕ полноценная ночная самоочистка из будущего ТЗ — нет консолидации групп, нет авто-стопа простаивающих демонов, нет удаления stale `no_bypass`/`direct` записей; единственный эффект — домены пересчитываются, а DIRECT-вердикт приводит к GC воркером.
